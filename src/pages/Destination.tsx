    ...
    <div className="min-h-screen bg-[#fcf8f5] text-[#555] font-poppins text-[14px] pb-12">
      ...
      >
        ...
        <div className="absolute inset-0 flex flex-col justify-between py-8 max-w-7xl mx-auto w-full">
          ...
          <div className="text-white drop-shadow-md">
            <h1 className="font-display text-[26px] font-bold tracking-tight">{data.title}</h1>
      ...
      <main className="max-w-7xl mx-auto py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-10">
          ...
          {/* About Field */}
          <section>
            <h2 className="font-display text-[26px] font-bold text-slate-900 mb-4">About this Destination</h2>
          ...
          {/* Highlights Grid */}
          <section>
            <h2 className="font-display text-[26px] font-bold text-slate-900 mb-4">Highlights</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {data.highlights.map((highlight, idx) => (
                <div key={idx} className="flex items-center gap-3 bg-white border border-slate-100 rounded-lg p-4 shadow-sm">
          ...
          <section className="space-y-4">
            <h2 className="font-display text-[26px] font-bold text-slate-900">Reviews</h2>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-slate-800">{data.reviewsSummary.rating}</span>
              <span className="text-[12px] text-slate-400">({data.reviewsSummary.totalReviews.toLocaleString()} reviews)</span>
            </div>

            <div className="space-y-3 pt-2">
              {data.reviews.map((review) => (
                <div key={review.id} className="bg-white border border-slate-100 rounded-lg p-5 shadow-sm space-y-2">
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-full ${review.avatarColor} text-white font-bold flex items-center justify-center text-[12px] shadow-inner`}>
        ...
        <div className="space-y-6 lg:h-fit lg:sticky lg:top-6">
          ...
          <div className="bg-white border border-slate-100 rounded-xl p-6 shadow-sm text-center space-y-5">
            <div>
              <span className="text-[12px] text-slate-400 block mb-0.5">Starting from</span>
              <span className="text-3xl font-extrabold text-[#AF1D1D]">
                {data.pricing.currency}{data.pricing.amount.toLocaleString()}
              </span>
              <span className="text-[12px] text-slate-400 block mt-0.5">{data.pricing.period}</span>
            </div>
            ...
            <div className="border-t border-slate-100 divide-y divide-slate-100 text-[13px] text-slate-600">
              <div className="flex justify-between py-3">
                ...
                <span className="font-bold text-slate-800">{data.specs.altitude}</span>
              </div>
              <div className="flex justify-between py-3">
                <span className="text-slate-400 font-medium">Best Season</span>
                <span className="font-bold text-slate-800">{data.specs.bestSeason}</span>
              </div>
              <div className="flex justify-between py-3">
          ...
          <div className="bg-red-50 border border-red-100 rounded-xl p-5 space-y-3 text-[13px]">
            <h3 className="font-bold text-slate-800 text-[14px]">Emergency Contact</h3>
            <div className="space-y-1 text-slate-600 font-medium leading-relaxed">
              {data.emergencyContacts.map((contact, idx) => (
                <p key={idx}>
                  <span className="text-slate-400 font-normal">{contact.label}:</span> {contact.value}
                </p>
              ))}
            </div>
          </div>
